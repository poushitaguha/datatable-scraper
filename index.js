...
(async () => {
    await nightmare
        ...
        .wait('input[type="submit"]')                   // Wait for the "Google Search" button   
        .click('input[type="submit"]')                  // Click on the "Google Search" button
    await nightmare
        .evaluate(() => document.querySelector('.rc .r a').click()) // Select the first result (https://datatables.net) and click on it
    await nightmare
        .wait('select[name="example_length"]')          // Wait for the table to load
